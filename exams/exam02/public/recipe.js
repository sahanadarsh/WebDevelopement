...
var loggedInHomePage = document.querySelector('.logged-in-home-page');
...
var backToHomePage = document.querySelector('.back-to-homepage');
var errorPage = document.querySelector('.status');
var titleAuthorHeading = document.querySelector('.title-author-heading');
...
function renderRecipeList() {
  ...
  recipeList.innerHTML = "<ul class=\"recipes\">" + Object.keys(recipes).map(function (key) {
    ...
    return "\n\t\t\t\t<li class=\"recipe-list\">\n\t\t\t\t\t<div class=\"recipe\">\n\t\t\t\t\t\t<a data-id=\"".concat(key, "\" class=\"recipe-title\" href=\"https://www.recipes.com\" title=\"click to get detailed recipe\">").concat(recipe.title, "</a>\n\t\t\t\t\t\t<span data-id=\"").concat(key, "\" class=\"recipe-author\">").concat(recipe.author, "</span>\n\t\t\t\t\t</div>\n\t\t\t\t</li>\n\t\t\t");
  }).join('\n') + "</ul>";
...
addRecipePage.addEventListener('click', function (e) {
  ...
  if (e.target.classList.contains('recipe-submit-button')) {
    var recipeTitle = addRecipePage.querySelector('.new-recipe-title').value;
    ...
    Object(_services__WEBPACK_IMPORTED_MODULE_0__["fetchAddRecipe"])(recipeTitle, recipeIngredients, recipeInstructions).then(function (recipe) {
      appState.error = '';
      appState.isLoggedIn = true;
...
loggedInHomePage.addEventListener('click', function (e) {
  e.preventDefault();
